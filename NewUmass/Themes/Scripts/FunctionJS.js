...
function IsEditdata(id) {
    ...
    if (id == undefined || id == "") {
        isOK = false;
        showTipsMsg("很抱歉，您当前未选中任何一行！", 2000, 3);
    } else if (id.split(",").length > 1) {
        isOK = false;
        showTipsMsg("很抱歉，一次只能选择一条记录！", 2000, 3);
    }
    return isOK;
}
...
/*parm：      条件参数
--------------------------------------------------*/
function delWinConfig(url, parm, count) {
    DeleteWinConfig(url, parm, "注：删除操作不可恢复，您确定要继续么？", count);
}

function DeleteWinConfig(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
        ...
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    ...
                    showTipsMsg("删除失败，请稍后重试", 2000, 5);
                } else {
                    showTipsMsg(rs, 2000, 3);
                }
            });
        }
    });
...
function GrantConfig(url, parm, count) {
    DoConfigs(url, parm, "注：取消授权后不可使用，您确定要继续么？", count);
}

function ExamineConfig(url, parm, count) {
    DoConfigs(url, parm, "注：您确定要审核么？", count);
}
...
/*parm：      条件参数
--------------------------------------------------*/
function doConfig(url, parm, count, msg) {
    DoConfigs(url, parm, "您确定" + msg + "吗？", count);
}

function DoConfigs(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("成功执行 " + count + " 笔记录。", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("失败，请稍后重试", 2000, 5);
                } else {
                    showTipsMsg(rs, 2000, 3);
                }
            });
...
function MsgConfigs(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("成功分配 " + count + " 笔订单。", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("失败，请稍后重试", 2000, 5);
                } else {
...
obj：ID，rowCode：列号
**/
function GetPqGridArrRowValue(obj_ID) {
    var selectionArray = $(obj_ID).pqGrid("selection", { type: 'row', method: 'getSelection' });//多行数据
    var GUIDList = '';
    if (selectionArray.length > 0) {
        var DM = $(obj_ID).pqGrid("option", "dataModel");
        var data = DM.data;
        for (var i = 0; i < selectionArray.length; i++) {
            GUIDList = GUIDList + ",'" + data[selectionArray[i].rowIndx][0] + "'";
        }
        if (GUIDList.length > 0)
            GUIDList = GUIDList.substring(1);//GUID拼出的字符串
        return GUIDList;
    } else {
        return null;
    }
...
//带回调参数的弹窗
function openDialogCall(url, _id, _title, _width, _height, left, top) {
    art.dialog.open(url, {
        id: _id,
        title: _title,
        width: _width,
