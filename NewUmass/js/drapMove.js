        ...
        handle: function () {
            ...
            $(document).on('mousedown.DR', This.options.childClass, function (e) {
                ...
                if ($p.attr('DR_replace')) {// 拖动交换
                    ...
                    // 占位
                    $("<i id='holder1' class='DR_holder'></i>").css({
                        width: $p.outerWidth(),
            ...
            $(document).on('mouseup.DR', function () {
                ...
                if (window.DR_replace) {// 拖动交换
                    ...
                    var posheight = $that[0].scrollHeight;
                    var poswidth = $that[0].scrollWidth;
                    var posY = $that.offset().top;
                    var maxW = $("#dropContent2").offset().left;
                    var maxHeight = $("#dropContent2")[0].scrollHeight;
                    var maxWidth = $("#dropContent2")[0].scrollWidth;
                    var maxH = $("#dropContent2").offset().top;
                    if (posX <= 0) {
                        $('[DR_move]').remove();
                        $("#holder1").remove();
                        ...
                        $all.each(function (i) {
                            ...
                            if (left < col_c && left_w > col_c && top < row_c && top_h > row_c) {
                                ...
                                $that.stop().animate({
                                    ...
                                    $that.css('left', '').css('top', '').removeAttr('DR_move');
